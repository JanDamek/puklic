...
/**
 * Single owner of the UDP receive loop. Reads raw RTP packets from [transport], peeks at the
 * RTP payload type without decrypting, and forwards the raw bytes to either the audio or
 * video channel. Each consumer pulls packets via [audioPackets] / [videoPackets].
 *
...
internal class VoicePacketDispatcher(
    ...
    private suspend fun runLoop() {
        while (isActive()) {
            ...
            if (packet.size < RtpPacket.HEADER_SIZE) continue
