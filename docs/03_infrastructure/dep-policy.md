...
| `:shared:voice-codec` | desktop today (via `:shared:voice` `api`); direct iOS / macOS consumers per FP-4..6 | Apache-2.0, **BSD-3-Clause** (iOS only), MIT (BouncyCastle on jvmMain) | KMP-wide Discord voice transport codec: `AeadCipher` interface, `NonceGenerator`, `RtpPacket`, `VoicePacketCodec`, `OpusCodecFactory` (FP-4), `IpDiscovery` + `JitterBuffer` (FP-14h-2a), `XChaCha20Poly1305` expect/actual with iOS CryptoKit cinterop (FP-14h-2c), and — since FP-14h-2e (issue #69) — the voice-gateway machinery (`VoiceOp`, `VoiceGatewayPayload`, `VoiceGatewayTransport`, `VoiceGatewayConnection`, `KtorVoiceGatewayTransport`, `VoiceWsTransportFactory`), video packetisers (`Vp8Packetiser`, `H264Depacketizer`, `H264Fragmenter`, `VideoFrameFragmenter`), `SoundshareAudioRtpSender`, `VoicePacketDispatcher`, `AudioCapture` / `AudioPlayback` interfaces, JVM `JavaSound*` impls, `CapturePipeline`. Symbols are marked `@PuklicVoiceCodec` (opt-in `@RequiresOptIn.Level.ERROR`) so external consumers cannot couple to them — `:shared:voice` and `:desktop:app` opt-in module-wide via `languageSettings`. iOS `actual` links the prebuilt `libs/Opus.xcframework` (upstream libopus 1.5.2, BSD-3-Clause — App Store compatible). JVM `actual` keeps using FFmpeg-GPL JavaCPP for libopus (desktop only — never reaches the iOS classpath). Extraction rationale: `architect-reports/2026-05-29-fp1-voice-codec-extraction.md`, `architect-reports/2026-05-29-fp4-ios-opus-libopus.md`, `architect-reports/2026-05-29-fp14h-1-v2-voice-gateway-redesign.md`, `architect-reports/2026-05-29-fp14h-2e-implementation.md`. |
